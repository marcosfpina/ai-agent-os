...
async fn main() -> Result<()> {
    ...
    info!("📊 Initial state: running={}, hyprland_connected={}", 
        state.running, 
        state.hyprland_connected
    );
    ...
    match agent.get_metrics().await {
        Ok(metrics) => {
            ...
            info!("  Temperature: {:.1}°C (max: {:.1}°C)", 
                metrics.thermal.avg_temp_celsius,
    ...
    match agent.get_logs(5).await {
        Ok(logs) => {
            ...
            for log in logs.iter().take(5) {
                info!("  [{:?}] {}", log.priority, log.message.chars().take(100).collect::<String>());
            }
        }
        Err(e) => {
